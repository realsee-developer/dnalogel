...
- fix(tag-plugin-mask-3DBox-content): 解决标签 plugin 中 Mask 与 3DBox 类型标签不渲染 content 的问题，会导致业务端注册自定义面板无法渲染。
...
- fix(Sculpt): 更新相关 five 的获取方式

## 3.77.6

- feat(PanoTagPlugin): 支持 Mask，3DBox 标签

## 3.77.5

- feat(DigitalPerformancePlugin): 动画演绎增加道具绑定

## 3.77.4

- feat(Sculpt): 增加 sculpt 立方体，多棱柱编辑进行中对外的状态
...
- feat(panoTagPlugins): infive 为 0 时，opacity 不会受到 modelOpacity 的影响

## 3.74.2

- feat(Sculpt): 修复 point 的 label 遮挡 point 位置问题

## 3.74.1

- feat(PanoTagPlugin): 修改 mouseenter 为 pointerenter

## 3.74.0

- feat(PanoTagPlugin): pointTag 增加法线, 默认显示, 标签数据有 normal 和 normalLen 时显示法线
