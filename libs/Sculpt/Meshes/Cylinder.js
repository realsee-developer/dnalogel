...
class F extends b {
  ...
  setPoints(t) {
    var s, r;
    if (this._topCenter = (s = C(t.topCenter)) != null ? s : this.topCenter, this._bottomCenter = (r = C(t.bottomCenter)) != null ? r : this.bottomCenter, this._radius = f(t.radius) ? t.radius : this.radius, this._topCenter && this._bottomCenter) {
      const m = this._topCenter.clone().sub(this._bottomCenter).normalize();
      this.bottomCircle.setPoints({ center: this._bottomCenter, normal: m, radius: this._radius }), this.setTopCenter(this._topCenter);
    }
    this.needsRender = !0;
  }
  setTopCenter(t) {
