...
class _ extends Y {
  ...
  scale(t) {
    ...
    const I = new o().addVectors(n[0], n[2]).divideScalar(2), { position: k, direction: z } = f, D = new o().subVectors(n[1], n[0]), y = new o().subVectors(n[3], n[0]), V = (r) => {
      ...
      if (!this.startInfo)
        return i;
      const e = (r === "ew" ? D : y).clone(), c = k.clone().projectOnVector(e), b = g.clone().projectOnVector(e), M = I.clone().projectOnVector(e);
      if (b.equals(c))
        return i;
      if (r === "ns" && typeof this.startInfo.dragPointInCenterBottom != "boolean") {
        const l = new o().subVectors(b, M), P = l.x < 0 && e.x >= 0, w = l.y < 0 && e.y >= 0, O = l.z < 0 && e.z >= 0;
