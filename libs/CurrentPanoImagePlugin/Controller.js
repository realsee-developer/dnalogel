...
import { tweenProgress as M } from "../shared-utils/animationFrame/BetterTween.js";
...
import { CURRENT_PANO_IMAGE_DEFAULT_IMAGE as P } from "../shared-utils/url/defaultUrls.js";
...
class Be extends A {
  constructor(e, i) {
    ...
    o(this, "onMeshVisibleChange", (e) => {
      ...
      this.mesh.material.setValues({ opacity: 0 }), this.mesh.visible = !0, this.opacityAnimeTween = M(1e3).onUpdate(({ progress: t }) => {
        var s;
  ...
  setState(e, i = {}) {
    ...
    if (this.updateState(e, s), e.enabled !== void 0 && t.enabled !== e.enabled && (e.enabled ? this.checkMsg.pluginDisabled = "" : this.checkMsg.pluginDisabled = "调用 setState 禁用", e.enabled ? this._enable({ userAction: s }) : this._disable({ userAction: s })), e.visible !== void 0 && t.visible !== e.visible) {
      const d = { userAction: s, anime: { duration: 500 } };
      e.visible ? this.checkMsg.pluginHidden = "" : this.checkMsg.pluginHidden = "调用 setState 隐藏", e.visible ? this._show(d) : this._hide(d);
    }
  }
  /** 更改插件 Config
  ...
  updateConfig(e, i = {}) {
    ...
    this._config = s, t.imageURL !== s.imageURL && this.reloadTexture(s.imageURL), t.yOffset !== s.yOffset && this.updateMeshPosition(this.five.getCurrentState().panoIndex), t.yRotate !== s.yRotate && this.updateMeshQuaternion(s.yRotate), t.width !== s.width && (this.mesh.geometry = new h.PlaneBufferGeometry(s.width, s.width)), this.hooks.emit("configChange", { prevConfig: t, config: s, userAction: (d = i.userAction) != null ? d : !0 });
  }
  /** 问什么看不到模型 */
  __whyCantSeeMesh() {
    if (this.disposed)
