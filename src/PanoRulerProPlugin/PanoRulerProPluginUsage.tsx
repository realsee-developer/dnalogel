...
const PanoRulerProPluginUsage = (props: PanoRulerProPluginUsePropTypes) => {
  ...
  const panoRulerProData = useFetchDatas(DATA_TYPES.PANO_RULER_PRO_PLUGIN_SERVER_DATA, '80o024DE2xyva3j5BE', 'real')
  ...
  return (
    <Box>
      <Paper sx={{ position: 'fixed', top: '10px', right: '10px', backgroundColor: 'transparent' }}>
        <Button onClick={handleRulerEnable}>{rulerEnable ? OPERATE_OPTIONS.CLOSE : OPERATE_OPTIONS.OPEN}</Button>
      </Paper>
    </Box>
  )
}
