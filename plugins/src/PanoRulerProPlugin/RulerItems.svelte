...
import { nextFrame } from 'src/shared-utils/animationFrame';
import equal from 'src/shared-utils/equal';
import { intersectionOfLine } from 'src/shared-utils/math/planimetry';
import throttle from 'src/shared-utils/throttle';
import { onDestroy, onMount } from 'svelte';
import { Vector3 } from 'three';
import RulerItem from './RulerItem.svelte';
import type { PanoRulerProp, RulerItemProp } from './typings';
  let panoRulerProp: PanoRulerProp
  let rulerItemProp: RulerItemProp[] = []

  const { five, rulerDatas } = panoRulerProp
  ...
  const getRuleScreenIntr = (startPoint: { x: number; y: number }, endPoint: { x: number; y: number }) => {
    const screenLines = [
      [
        { x: 0, y: 0 },
        { x: canvasWidth, y: 0 },
  ...
  const getPositionAndVisible = (start: Vector3, end: Vector3, visibleOrigin: Vector3) => {
    ...
    const intr: any = getRuleScreenIntr({ x: ~~startLeft, y: ~~startTop }, { x: ~~endLeft, y: ~~endTop })
  ...
  const updatePanoRulerData = () => {
    ...
    const data = rulerData.lines.map((line) => {
      ...
      const end = line.end
      const { startLeft, startTop, distance, deg, visible, labelOffset, ruleLength } = getPositionAndVisible(
      ...
      return {
        ...
        labelOffset,
        visible,
        labelElement: panoRulerProp.options.distanceText!(ruleLength),
      }
    })
